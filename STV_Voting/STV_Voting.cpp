...
// Configure handling of surplus ballots with no next preference
enum class SurplusNoNextPolicy { Resplit, Exhaust };
static SurplusNoNextPolicy g_SurplusNoNextPolicy = SurplusNoNextPolicy::Resplit;

static void configureSurplusNoNextPolicyFromEnv()
{
#ifdef _WIN32
 char* v = nullptr;
 size_t len =0;
 if (_dupenv_s(&v, &len, "STV_SURPLUS_NO_NEXT") !=0 || v == nullptr) return;
 std::string s(v);
 free(v);
#else
 const char* v = std::getenv("STV_SURPLUS_NO_NEXT");
 if (!v) return;
 std::string s(v);
#endif
 for (char& ch : s) ch = static_cast<char>(std::toupper(static_cast<unsigned char>(ch)));
 if (s == "EXHAUST" || s == "EXHAUSTED" || s == "EXHAUSTION") {
 g_SurplusNoNextPolicy = SurplusNoNextPolicy::Exhaust;
 } else if (s == "RESPLIT" || s == "SPLIT" || s == "REDISTRIBUTE") {
 g_SurplusNoNextPolicy = SurplusNoNextPolicy::Resplit;
 }
}

// Multi-seat Single Transferable Vote

// Step1: Calculate the election quota
double calculateQuota(int validBallots, int seats);

// Step2: Count first-choice votes
std::map<std::string, double> countFirstChoices(const std::vector<std::vector<std::string>>& ballots);

// Step3: Elect candidates who meet the quota

// Step4: Transfer surplus
...
// Step5: Eliminate the lowest-vote candidate

// Step6: Transfer votes from the eliminated candidate
...
// Helper to clamp displayed counts for candidates marked as Elected to the quota
...
// Rule: ballot goes to top continuing preference; if none remains, split1.00
// equally among all continuing candidates, floored to2 decimals per share.
 ...
 const std::set<std::string>& continuing)
{
 ...
 // No next preference -> split equally among all continuing candidates
 const size_t m = continuing.size();
 if (m ==0) continue;
 const double share = floor2(1.0 / static_cast<double>(m));
 if (share <=0.0) continue; // nothing to add if share floors to0.00
...
// - if they have a next continuing preference, transfer1.00 to that candidate
// - else split1.00 equally among all remaining continuing candidates (floored per share)
...
std::string runSingleSeatElection(const std::vector<std::vector<std::string>>& ballots)
{
 ...
 // §11D: if no surplus transfers this phase, use raw2nd/3rd/...
 ...
 const std::map<std::string, double>& voteCounts)
{
 ...
 const int denom = (g_SurplusNoNextPolicy == SurplusNoNextPolicy::Resplit)
 ? (transferable + noNextCount)
 : transferable;
 if (denom ==0) return result;

 // Per-ballot value depends on policy
 ...
 //2) Each “no next” ballot handling per policy
 if (g_SurplusNoNextPolicy == SurplusNoNextPolicy::Resplit && noNextCount >0) {
...
std::map<std::string, double> countFirstChoices(const std::vector<std::vector<std::string>>& ballots)
{
 ...
 // Pre-initialize all candidates observed in any position to0.0
 for (const auto& b : ballots) {
 for (const auto& c : b) {
 ...
 const std::set<std::string>& elected)
{
 ...
 //1) next recipients get full perBallot times their count
 std::map<std::string, int> nextCounts;
 for (const auto& nxt : nextRecipients) { nextCounts[nxt]++; }
 for (const auto& [rcpt, cnt] : nextCounts) {
 voteCounts[rcpt] += perBallot * static_cast<double>(cnt);
 }

 //2) policy for “no next”
 if (g_SurplusNoNextPolicy == SurplusNoNextPolicy::Resplit && noNextCount >0) {
 std::vector<std::string> cont;
 for (const auto& kv : voteCounts)
 ...
 for (const auto& c : cont) { voteCounts[c] += split; }
 }
 }
 }
 }
 ...
 const std::set<std::string>& elected)
{
 ...
 // Per-ballot value = eliminated total divided by number of transferable ballots, floored to2 decimals
...
std::set<std::string> runMultiSeatElection(const std::vector<std::vector<std::string>>& ballots, int seats)
{
 ...
 // CSV header and initial snapshot (Round0)
 printCsvHeader();
 int round =0; 
 {
 std::map<std::string, double> noneAmt;
 ...
 // §11D: use2nd/3rd/... preferences first
 ...
 // Clamp all candidates marked as Elected in this display to the quota
...
for (const auto& cand : newly) {
 ...
 // Clamp elected candidates to quota for display consistency
 auto displayCounts = makeDisplayCountsClampedToQuota(voteCounts, elected, quota);
 ...
 std::map<std::string, std::vector<std::pair<std::string, double>>> noneSrc;
 auto displayCounts = makeDisplayCountsClampedToQuota(voteCounts, elected, quota);
 printCsvRound(round++, displayCounts, elected, allCandidates, noneAmt, noneSrc);
 }
...
int inputNumberOfSeats()
{
 ...
 if (v <=0 || v >1000000) {
 std::cout << "Seats must be between1 and1,000,000. Try again.\n";
 continue;
 }
...
int main()
{
 configureSurplusNoNextPolicyFromEnv();
 for (;;)
 {
...
#else
 std::string line;
 if (!std::getline(std::cin, line)) return0;
 ...
 // After the prompt loop, either rerun or exit
